...
if __name__ == "__main__":
    ...
    mask = maskLoader.load(visualize=False)
    

    CYLINDER_CENTER = [N_X //5,  N_Y//2, N_Z//2]
    CYLINDER_RADIUS = N_Y // 5
    mask = torch.tensor(np.sqrt(
        (X - CYLINDER_CENTER[0])**2 + (Y - CYLINDER_CENTER[1])**2 + (Z - CYLINDER_CENTER[2])**2
        ) < CYLINDER_RADIUS).to(DEVICE)
    L = CYLINDER_RADIUS

   
    # Visualizer and Solver
    vis = FluidVisualizer3D((N_X, N_Y, N_Z), saveOutput=True, outputFile="simulation.mp4", maxOpacity=0.2)
    lbm = LBMSolver3D(Ci, WEIGHTS, REYNOLD_NUMBER, N_DVELOCITIES, torch.ones((N_X, N_Y, N_Z)).to(DEVICE), 
                      DIRECTIONAL_VELOCITIES, RIGHT_VELOCITY, LATTICES, OPPOSITE_LATTICES, L)
